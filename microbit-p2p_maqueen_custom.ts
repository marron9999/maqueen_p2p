...
namespace maqueen {
    ...
    //    export enum Patrol1 {
    //        //% blockId="patrol1Left" block="left"
    //        Left = 0x10,
    //        //% blockId="patrol1Right" block="right"
    //        Right = 0x20
    //    }
    ...
    //
    //    %%% weight=20
    //    %%% blockId=motor_Motortop block="motor |%Motor stop"
    //    %%% Motor.fieldEditor="gridpicker" Motor.fieldOptions.columns=2 
    //    export function Motortop(Motor: Motor): void {
    //        let buf = pins.createBuffer(3);
    //        if (Motor == 0) {
    //            buf[0] = 0x00;
    //            buf[1] = 0;
    ...
    export function readPatrol(patrol: Patrol): number {
        if (patrol == Patrol.Left) {
            return pins.digitalReadPin(DigitalPin.P13)
        } else if (patrol == Patrol.Right) {
    ...
    //% weight=20
    //% blockId=writeLED block="LEDlight |%led turn |%Switch"
    //% led.fieldEditor="gridpicker" led.fieldOptions.columns=2 
    //% Switch.fieldEditor="gridpicker" Switch.fieldOptions.columns=2
    export function writeLED(led: LED, Switch: Switch): void {
        if (led == LED.Left) {
            pins.digitalWritePin(DigitalPin.P8, Switch)
        } else if (led == LED.Right) {
            pins.digitalWritePin(DigitalPin.P12, Switch)
        } else {
            return
        }
    }
